...
static int
dt_pid_has_jump_table(struct ps_prochandle *P, dtrace_hdl_t *dtp, uint8_t *text,
    fasttrap_probe_spec_t *ftp, const GElf_Sym *symp)
{
	ulong_t i;
	int size;
	...
	if (dt_pid_has_jump_table(P, dtp, text, ftp, symp)) {
		for (i = 0, end = ftp->ftps_size; i < end; i += size) {
			...
			if (text[i] == DT_LEAVE && text[i + 1] == DT_RET) {
			    ...
			    text[i + 2] == DT_POPL_EBP &&
			    text[i + 3] == DT_REP && text[i + 4] == DT_RET) {
		...
		for (i = 0, end = ftp->ftps_size; i < end; i += size) {
			...
			if (size == 5 && text[i] == DT_JMP32 &&
			    symp->st_size <= (uintptr_t)(i + size +
						 *(int32_t *)&text[i + 1]))
				goto is_ret;

			/* 8-bit displacement jmp outside of the function */
			if (size == 2 && text[i] == DT_JMP8 &&
			    symp->st_size <=
