...
usage(void)
{
	(void)fprintf(stderr,
	    ...
	    progname, progname, (int)strlen(progname), " ", progname, progname);
}
...
wip_save_work(workqueue_t *wq, wip_t *slot, int slotnum)
{
	...
	wq->wq_lastdonebatch++;
	pthread_cond_signal(
	    &wq->wq_wip[(slotnum + 1) % wq->wq_nwipslots].wip_cv);
...
copy_ctf_data(char *srcfile, char *destfile, int keep_stabs)
{
	...
	write_ctf(srctd, destfile, tmpname,
	    CTF_COMPRESS | CTF_SWAP_BYTES | keep_stabs);
...
wq_init(workqueue_t *wq, int nfiles)
{
	...
	nslots = MIN(nslots,
	    (nfiles + wq->wq_maxbatchsz - 1) / wq->wq_maxbatchsz);
...
main(int argc, char **argv)
{
	 ...
	 */
	if (read_ctf(ifiles, nifiles, NULL, merge_ctf_cb, &wq, require_ctf) ==
	    0) {
	...
	if (uniqfile != NULL || withfile != NULL) {
		...
		if (read_ctf(&reffile, 1, reflabel, read_ctf_save_cb, &reftd,
			require_ctf) == 0) {
